...
public class ClientScreen extends Screen {
    ...
    public ClientScreen() {
        super(Component.literal("Custom Client"));
    }
    ...
    public void render(GuiGraphics g, int mouseX, int mouseY, float delta) {
        ...
        for (ModuleCategory c : ModuleCategory.values()) {
            ...
            g.fill(x + 10, cy - 3, x + 105, cy + 14,
                    selected ? 0xFF3A5FFF : 0xFF242424);
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        for (ModuleCategory c : ModuleCategory.values()) {
            if (mouseX >= x + 10 && mouseX <= x + 105
                    && mouseY >= cy - 3 && mouseY <= cy + 14) {
                selectedCategory = c;
                return true;
            }
            cy += 22;
        ...
        for (Module m : ClientCore.getInstance()
                .getModuleManager()
                .getModulesByCategory(selectedCategory)) {

            if (mouseX >= mx && mouseX <= mx + 220
                    && mouseY >= my && mouseY <= my + 30) {
                m.toggle();
